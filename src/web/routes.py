...
def register_routes(app):
    ...
    def api_current():
        ...
        try:
            ...
            # Registro agregado para totais de energia (panel_id=None com energy_kwh_total preenchido)
            aggregate = next((d for d in data if d.panel_id is None and d.energy_kwh_total is not None), None)
    ...
    def api_alerts():
        """Retorna alertas recentes. ?all=1 inclui resolvidos. ?limit=N limita quantidade."""
        try:
    ...
    def api_insights():
        ...
        try:
            ...
            aggregate = next(
                (d for d in reversed(data_today) if d.panel_id is None and d.energy_kwh_total is not None),
                None
            )
            lifetime_kwh = float(aggregate.energy_kwh_total) if aggregate else 0.0

            # Financial & CO2 estimates (Brazil averages)
            TARIFF_BRL    = 0.80   # R$/kWh
            CO2_FACTOR    = 0.5    # kg CO2 avoided per kWh
            INSTALLED_KW  = 15.95  # kWp nominal
            ...
            if telemetry and telemetry.time_series:
                ...
                if times and powers:
                    ...
                    end_time = next(
                        (times[i] for i in range(len(powers)-1, -1, -1) if powers[i] > 0 and i < len(times)), None
                    )
                    ...
                    # Estimate solar day duration in minutes
            ...
            batch = repository.get_latest_inverter_batch_for_date(today)
            inv_avg_power = {}  # uid -> avg W (non-zero)
            if batch and batch.power_data:
                uid_pts = {}
                for ch_key, values in batch.power_data.get('power', {}).items():
                    uid = ch_key.rsplit('-', 1)[0]
            ...
            for rec in sorted(summaries or [], key=lambda r: r.inverter_uid):
                ...
                # Channel asymmetry: (max-min)/max × 100
            ...
            # Rank within series (by today_kwh)
            by_series = {}
